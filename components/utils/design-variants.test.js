...
test("exposes all design variants with stable labels and defaults", async (t) => {
	...
	assert.deepEqual(harness.getDesignVariants(), { advancedTimeline: false, agentSessionColumnResizing: false, kanbanBackground: false, manualLink: false, panel: false, sessionBloom: true, sessionProximity: true, sessionStroke: true, "simple-views": true, simpleKanban: true });
...
test("exposes Dragging off by default", async (t) => {
	const harness = await loadDesignVariantsHarness(t);
	...
	assert.equal(harness.getDefaultDesignVariants().agentSessionColumnResizing, false);
...
// Both glow layers ship on, so the menu is how you turn the shipped behaviour
// off to compare — not how you turn a hidden feature on.
test("exposes the three session-chrome layers on by default and independently", async (t) => {
	const harness = await loadDesignVariantsHarness(t);
	...
	for (const id of ["sessionStroke", "sessionBloom", "sessionProximity"]) {
		assert.equal(harness.getDefaultDesignVariants()[id], true, `${id} defaults on`);
	}
	...
	assert.equal(harness.getDesignVariants().sessionProximity, true);

	harness.setDesignVariant("sessionStroke", false);
	assert.equal(harness.getDesignVariants().sessionStroke, false);
	assert.equal(harness.getDesignVariants().sessionProximity, true);
});

// A stored payload written before these ids existed has neither key. It must
// adopt the on defaults rather than resolving them to off, or an existing user
// would silently lose the effect.
test("a pre-existing stored payload keeps the session-chrome defaults", async (t) => {
	const harness = await loadDesignVariantsHarness(t);

	globalThis.localStorage.setItem(
		harness.DESIGN_VARIANTS_STORAGE_KEY,
		JSON.stringify({ advancedTimeline: false, agentSessionColumnResizing: false, kanbanBackground: false, manualLink: false, panel: true, "simple-views": true, simpleKanban: true, schemaVersion: 2 }),
	);
...
test("snapshot getters keep a stable identity for useSyncExternalStore", async (t) => {
	...
	assert.equal(harness.getDesignVariants(), harness.getDesignVariants());
	assert.deepEqual(harness.getDefaultDesignVariants(), { advancedTimeline: false, agentSessionColumnResizing: false, kanbanBackground: false, manualLink: false, panel: false, sessionBloom: true, sessionProximity: true, sessionStroke: true, "simple-views": true, simpleKanban: true });
});
...
test("toggling a variant persists it and notifies subscribers exactly once", async (t) => {
	...
	assert.deepEqual(harness.getDesignVariants(), { advancedTimeline: false, agentSessionColumnResizing: false, kanbanBackground: false, manualLink: false, panel: true, sessionBloom: true, sessionProximity: true, sessionStroke: true, "simple-views": true, simpleKanban: true });
	assert.deepEqual(seen, [{ advancedTimeline: false, agentSessionColumnResizing: false, kanbanBackground: false, manualLink: false, panel: true, sessionBloom: true, sessionProximity: true, sessionStroke: true, "simple-views": true, simpleKanban: true }]);
	assert.deepEqual(
		JSON.parse(localStorage.getItem(harness.DESIGN_VARIANTS_STORAGE_KEY)),
		{ advancedTimeline: false, agentSessionColumnResizing: false, kanbanBackground: false, manualLink: false, panel: true, sessionBloom: true, sessionProximity: true, sessionStroke: true, "simple-views": true, simpleKanban: true, schemaVersion: 2 },
	);
	...
	assert.deepEqual(
		JSON.parse(localStorage.getItem(harness.DESIGN_VARIANTS_STORAGE_KEY)),
		{ advancedTimeline: false, agentSessionColumnResizing: false, kanbanBackground: false, manualLink: false, panel: false, sessionBloom: true, sessionProximity: true, sessionStroke: true, "simple-views": true, simpleKanban: true, schemaVersion: 2 },
	);
});
...
test("no-op writes and no-op hydrations never notify subscribers", async (t) => {
	...
	harness.hydrateDesignVariants({ advancedTimeline: false, agentSessionColumnResizing: false, kanbanBackground: false, manualLink: false, panel: true, sessionBloom: true, sessionProximity: true, sessionStroke: true, "simple-views": true, simpleKanban: true });
	assert.equal(notifications, 1);
...
test("hydration adopts a stored state without rewriting storage", async (t) => {
	...
	assert.deepEqual(harness.readStoredDesignVariants(), { advancedTimeline: false, agentSessionColumnResizing: false, kanbanBackground: false, manualLink: false, panel: true, sessionBloom: true, sessionProximity: true, sessionStroke: true, "simple-views": true, simpleKanban: true });
...
test("normalises unknown keys and non-boolean values in stored payloads", async (t) => {
	...
	assert.deepEqual(harness.readStoredDesignVariants(), { advancedTimeline: false, agentSessionColumnResizing: false, kanbanBackground: false, manualLink: false, panel: false, sessionBloom: true, sessionProximity: true, sessionStroke: true, "simple-views": true, simpleKanban: true });
	...
	assert.deepEqual(harness.readStoredDesignVariants(), { advancedTimeline: false, agentSessionColumnResizing: false, kanbanBackground: false, manualLink: false, panel: false, sessionBloom: true, sessionProximity: true, sessionStroke: true, "simple-views": false, simpleKanban: true });
	...
	localStorage.setItem(
		"ui-design-variants",
		JSON.stringify({ panel: true, sessionBloom: true, sessionProximity: true, sessionStroke: true, "simple-views": true, simpleKanban: false }),
	);
	assert.deepEqual(harness.readStoredDesignVariants(), { advancedTimeline: false, agentSessionColumnResizing: false, kanbanBackground: false, manualLink: false, panel: true, sessionBloom: true, sessionProximity: true, sessionStroke: true, "simple-views": true, simpleKanban: true });
	...
	assert.deepEqual(harness.readStoredDesignVariants(), { advancedTimeline: false, agentSessionColumnResizing: false, kanbanBackground: false, manualLink: false, panel: false, sessionBloom: true, sessionProximity: true, sessionStroke: true, "simple-views": true, simpleKanban: false });
	...
	for (const value of ["true", 1, {}, [], null]) {
		...
		assert.deepEqual(
			...
			{ advancedTimeline: false, agentSessionColumnResizing: false, kanbanBackground: false, manualLink: false, panel: false, sessionBloom: true, sessionProximity: true, sessionStroke: true, "simple-views": true, simpleKanban: true },
...
test("toggling Simple kanban preserves Panel and Simple views", async (t) => {
	...
	assert.deepEqual(harness.getDesignVariants(), { advancedTimeline: false, agentSessionColumnResizing: false, kanbanBackground: false, manualLink: false, panel: false, sessionBloom: true, sessionProximity: true, sessionStroke: true, "simple-views": true, simpleKanban: false });
	assert.deepEqual(
		JSON.parse(localStorage.getItem(harness.DESIGN_VARIANTS_STORAGE_KEY)),
		{ advancedTimeline: false, agentSessionColumnResizing: false, kanbanBackground: false, manualLink: false, panel: false, sessionBloom: true, sessionProximity: true, sessionStroke: true, "simple-views": true, simpleKanban: false, schemaVersion: 2 },
	);
