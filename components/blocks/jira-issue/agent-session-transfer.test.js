...
test("Jira issue drag pill floats on overlay elevation, not a dead utility", () => {
	...
	assert.match(dragChipSource, /<HumanAgentAvatar[\s\S]*agent=\{agent\}[\s\S]*human=\{attributedBy\}[\s\S]*animate=\{animateIdentity \?\? isFusionSource\}[\s\S]*composition="horizontal-group"[\s\S]*attributionOrder=\{attributionOrder\}[\s\S]*sizePx=\{32\}/u);
