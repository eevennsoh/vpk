...
test("View picker exposes unselected filter-action submenus with a selected count and clear action", () => {
	...
	assert.match(VIEW_MENU_SOURCE, /working: \{ spinner: "experimental-avatar" \}/u);
	assert.match(
		VIEW_MENU_SOURCE,
		/className=\{MENU_LEADING_SPINNER_CLASS_NAME\}[\s\S]*label=""[\s\S]*size="xs"[\s\S]*variant="experimental-avatar"/u,
	);
...
test("Needs input button leads with the ADS question-circle icon and omits its count badge", () => {
	assert.match(
		VIEW_MENU_SOURCE,
